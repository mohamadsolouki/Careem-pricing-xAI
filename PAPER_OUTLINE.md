...
*Methodology:* Using Design Science Research (Peffers et al., 2007), we construct a high-fidelity synthetic mirror dataset of 150,000+ Dubai ride-hailing records incorporating spatial, temporal, event, and weather features, train an XGBoost price prediction model, and apply SHAP to generate local and global explanations.

*Findings:* The framework reveals that distance, time-of-day, active events, and weather are the dominant pricing drivers. A live interactive application demonstrates real-time explainable price estimation with plain-language breakdowns.
